...
module.exports.tempTrack = function (req, callback) {

	var url_to_convert = url.parse( req.url, true).query.url;
	if (url_to_convert !== "" && typeof url_to_convert !== 'undefined') {

		console.log(url_to_convert);

		var newTrack = new db.Track();
		newTrack.src = url_to_convert;
		newTrack.save( function (err) {
			if (!err) callback(newTrack);
			else callback(err);
		});
		...
		callback({
			error: "400",
			errorMsg: "No url was passed with the query"
		})
	}
};

module.exports.saveTrack = function (req, callback) {
	...
	if (url_to_convert !== "" && typeof url_to_convert !== 'undefined') {
		...
		var newTrack = new db.Track();
		newTrack.src = url_to_convert;
		newTrack.forUser = "testing";
		newTrack.users = [req.user.id];
		newTrack.type = "saved";
		newTrack.save(function(err){
			if (!err) callback(newTrack);
			else callback(err);
...
router.on("/auth", function (req, res) {
	...
	var query = querystring.parse(fullQuery);
